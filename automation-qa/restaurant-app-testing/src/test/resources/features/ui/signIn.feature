Feature: Verify the sign in of the user
  ...
  Scenario Outline: Verify missing email
    When the user enters the "<email>" and "<password>"
    And the user clicks on sign in button
  ...
  Scenario Outline: Verify whether the account is blocked after 3 invalid attempts
    ...
    Examples:
      |email                       | password           | error                 |
      |  sindhug@example.com       |  susha@W21         |Incorrect email or password. Try again or create an account.|
      |  sindhug@example.com       |  susha@W21         |Incorrect email or password. Try again or create an account.|
      |  sindhug@example.com       |  susha@W21         |Incorrect email or password. Try again or create an account.|
      |  sindhug@example.com       |  susha@W21         |Your account is temporarily locked due to multiple failed login attempts. Please try again later.|
