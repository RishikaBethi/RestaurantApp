...
//        logger.info("Initialized with clientId: " + clientId); // Keep for setup verification
//    }
//
...
//            valueMap.withString(":ln", lastName);
//
//            if (imageUrl != null && !imageUrl.isEmpty()) {
//                updateExpression.append(", imageUrl = :iu");
//                valueMap.withString(":iu", imageUrl);
//            }
//
...
//
//            // Reset failed attempts and lockout if they exist
//            Item userItem = usersTable.getItem("email", email);
...
//    @Inject
//    public ProfileService(CognitoIdentityProviderClient cognitoClient, DynamoDB dynamoDB,
//                          ObjectMapper objectMapper, String clientId, S3Client s3Client) {
//        this.cognitoClient = cognitoClient;
//        this.dynamoDB = dynamoDB;
//        this.usersTable = dynamoDB.getTable(System.getenv("USERS_TABLE"));
//        this.objectMapper = objectMapper;
...
//        logger.info("Initialized with clientId: " + clientId + ", bucketName: " + bucketName);
//    }
//
//    public APIGatewayProxyResponseEvent getUserProfile(APIGatewayProxyRequestEvent request) {
//        try {
...
//            }
//
//            // Validate Base64 string and upload to S3 if provided
//            String imageUrl = "";
//            if (base64encodedImage != null && !base64encodedImage.isEmpty()) {
//                try {
//                    byte[] imageBytes = Base64.getDecoder().decode(base64encodedImage);
//                    String fileName = "img/" + UUID.randomUUID().toString() + ".jpg"; // Assuming JPEG; adjust if needed
...
//                    imageUrl = String.format("https://%s.s3.eu-central-1.amazonaws.com/%s", bucketName, fileName);
//                } catch (IllegalArgumentException e) {
//                    return Helper.createErrorResponse(400, "Invalid Base64 encoded image data.");
//                } catch (Exception e) {
//                    logger.severe("Error uploading image to S3: " + e.getMessage());
//                    return Helper.createErrorResponse(500, "Error uploading image to S3: " + e.getMessage());
//                }
//            }
//
//            Item existingItem = usersTable.getItem("email", email);
...
//            StringBuilder updateExpression = new StringBuilder("SET ");
//            ValueMap valueMap = new ValueMap();
//            updateExpression.append("firstName = :fn, lastName = :ln, imageUrl = :img");
//            valueMap.withString(":fn", firstName)
//                    .withString(":ln", lastName)
//                    .withString(":img", imageUrl);
//
//            usersTable.updateItem(new UpdateItemSpec()
//                    .withPrimaryKey("email", email)
//                    .withUpdateExpression(updateExpression.toString())
